...
export default function BarsAndNightlife() {
  const { negrilGuides, guideSections } = useAppSelector(
    (state) => state.negril
  )
  const section = guideSections?.find((s: any) => s.pkid === 2)
  const guides = negrilGuides?.filter((c) => c.category === CATEGORIES.bars)
  ...
  //     (docs: any) => {
  //       setHeader(docs?.data()?.foodHeader)
  //     }
  //   )
  //   return () => {
  //     unsub()
  ...
  return (
    <Box id="bars" sx={{ scrollMarginTop: '100px' }}>
      <SectionHeader
        header={section}
        id={section?.pkid}
        title={section?.title}
