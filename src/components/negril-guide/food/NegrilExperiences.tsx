...
export default function NegrilExperiences() {
  const { negrilGuides, guideSections } = useAppSelector(
    (state) => state.negril
  )
  const section = guideSections?.find((s: any) => s.pkid === 9)
  const guides = negrilGuides?.filter(
    (c) => c.category === CATEGORIES.negrilExperiences
  )

  return (
    <Box id="negrilExperiences" sx={{ scrollMarginTop: '100px' }}>
      <SectionHeader
        header={section}
        id={section?.pkid}
        title={section?.title}
