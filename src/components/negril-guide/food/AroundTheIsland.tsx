...
export default function AroundTheIsland() {
  const { negrilGuides, guideSections } = useAppSelector(
    (state) => state.negril
  )
  const section = guideSections?.find((s: any) => s.pkid === 11)
  const guides = negrilGuides?.filter(
    (c) => c.category === CATEGORIES.aroundTheIsland
  )

  return (
    <Box id="aroundTheIsland" sx={{ scrollMarginTop: '100px' }}>
      <SectionHeader
        header={section}
        id={section?.pkid}
        title={section?.title}
