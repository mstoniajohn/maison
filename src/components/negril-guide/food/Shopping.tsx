...
export default function Shopping() {
  ...
  const section = guideSections?.find((s: any) => s.pkid === 8)
  const guides = negrilGuides?.filter((c) => c.category === CATEGORIES.shopping)

  return (
    <Box id="shopping" sx={{ scrollMarginTop: '100px' }}>
      <SectionHeader
        header={section}
        id={section?.pkid}
        title={section?.title}
      ...
      {!negrilGuides?.length ? (
        ...
        <Box className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {guides?.map((item: any) => (
            <NegrilCardItem
              body={item?.body}
              title={item?.title}
